import os

from robust_llm.batch_job_utils import run_multiple

EXPERIMENT_NAME = os.path.basename(__file__).replace(".py", "")
EXP_PREFIX = "-".join(EXPERIMENT_NAME.split("_")[:2])
HYDRA_CONFIG = "niki/0_DEBUG_niki_124"


base_model_name = "EleutherAI/pythia-14m"
n_max_parallel = 4
seed = 0

OVERRIDE_ARGS_LIST = [
    {
        "model.name_or_path": base_model_name,
        # Save to HF since we want to use these random token hardened models for
        # the beam search attack as well
        "training.force_name_to_save": f"{base_model_name[11:]}_{EXP_PREFIX}_imdb_random-token-1280_seed-{seed}-debug-0",  # noqa: E501
        # Defense uses random token 1280, attack uses gcg 10 iterations
        "training.seed": seed,
    },
    {
        "model.name_or_path": base_model_name,
        # Save to HF since we want to use these random token hardened models for
        # the beam search attack as well
        "training.force_name_to_save": f"{base_model_name[11:]}_{EXP_PREFIX}_imdb_random-token-1280_seed-{seed}-debug-1",  # noqa: E501
        # Defense uses random token 1280, attack uses gcg 10 iterations
        "training.seed": seed,
    },
]
N_MAX_PARALLEL = [n_max_parallel for _ in OVERRIDE_ARGS_LIST]


if __name__ == "__main__":
    run_multiple(
        EXPERIMENT_NAME,
        HYDRA_CONFIG,
        OVERRIDE_ARGS_LIST,
        N_MAX_PARALLEL,
        memory="60G",
        priority="normal-batch",
    )
