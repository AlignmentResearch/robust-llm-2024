...
from robust_llm.experiment_utils import (
    get_all_minimal_rounds_to_evaluate,
    get_start_median_and_end,
)
...
# Get a minimal range of rounds for each model size
evaluation_rounds = get_all_minimal_rounds_to_evaluate()
...
short_model_list = get_start_median_and_end(MODEL_GPU_MEMORY_CLUSTER_PARALLEL)
short_evaluation_rounds = get_start_median_and_end(evaluation_rounds)
...
OVERRIDE_TUPLES = [
    ...
    for (model, n_gpus, memory, cluster, parallel), evaluation_rounds_list in zip(
        short_model_list, short_evaluation_rounds
    )
