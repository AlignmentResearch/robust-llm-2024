...
GROUPS = [
    ...
    if path.startswith("oskar_009h")
    or path.startswith("oskar_009i")
    or path.startswith("oskar_009j")
]
...
def extract_attack_from_experiment_name(experiment_name):
    attack_names = ["gcg_10it", "gcg_30it", "gcg_100it", "pair", "few_shot", "zs"]
...
df = df.loc[
    ~df.attack_name.str.contains("gcg") | df.experiment_name.str.contains("oskar_008h")
]
df.head()
# %%
print(df.model_size.value_counts())
# %%
df[["adversary_size", "attack_success_rate", "attack_name"]].style.background_gradient(
    cmap="RdBu", axis=0
).format({"attack_success_rate": "{:.2f}", "adversary_size": "{:.1E}"})
# %%
...
ax.set_xlabel("Adversary Size")
ax.set_ylabel("Attack Success Rate")
ax.set_title("Attack Success Rate vs Adversary Size with Error Bars")
