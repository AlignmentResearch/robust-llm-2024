...
def version_path_to_timestamp_str(version_path: Path) -> str:
    """Gets timestamp out of a path generated by generate_model_save_path()."""
    match = re.match(r"\d{8}-\d{6}", version_path.name)
    assert match is not None, f"Invalid model revision version path: {version_path}"
    return match.group()
...
def mark_model_save_as_finished(model_save_directory: Path) -> None:
    ...
    with open(model_save_directory / "disk-write-timestamp.txt", "w") as f:
        ...
        f.write(version_path_to_timestamp_str(model_save_directory))
